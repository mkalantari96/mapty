...
class App {
  ...
  _hideForm() {
    ...
    setTimeout(() => (form.computedStyleMap.diplay = "grid"), 1000);
  }
  ...
  _newWorkout(e) {
    ...
    if (type === "running") {
      ...
      if (
        ...
        return alert("the input should be number");
      }
  ...
  _MoveToPopup(e) {
    const workoutChose = e.target.closest(".workout");
    if (!workoutChose) return;
    const workout = this.#workout.find(
      (work) => work.id === workoutChose.dataset.id
    );
